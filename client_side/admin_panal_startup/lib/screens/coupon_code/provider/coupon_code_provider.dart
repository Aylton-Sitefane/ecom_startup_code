  ...
  addCoupon() async {
    try {
      if (endDateCtrl.text.isEmpty) {
        SnackBarHelper.showErrorSnackBar('Please select end data!');
        return;
      }
      ...
      if (response.isOk) {
        ...
        if (apiResponse.success == true) {
          SnackBarHelper.showSuccessSnackBar('Coupon added sucessfuly');
  ...
  updateCoupon() async {
    try {
      ...
      if (response.isOk) {
        ApiResponse apiResponse = ApiResponse.fromJson(response.body, null);
        if (apiResponse.success == true) {
          SnackBarHelper.showSuccessSnackBar('Coupon updated sucessfuly');
        _dataProvider.getAllCoupons();

          clearFields();

        } else {
          SnackBarHelper.showErrorSnackBar('Error updating ${apiResponse.message}');
        }
      } else {
        SnackBarHelper.showErrorSnackBar('Error ${response.statusText}');
      }
  ...
  deleteCoupon(Coupon coupon) async {
    try {
      ...
      if (response.isOk) {
        ApiResponse apiResponse = ApiResponse.fromJson(response.body, null);
        if (apiResponse.success == true) {
          SnackBarHelper.showSuccessSnackBar('Deleted Sucessfully');
        _dataProvider.getAllCoupons();
        }
      } else {
        SnackBarHelper.showErrorSnackBar('Error ${response.statusText}');
