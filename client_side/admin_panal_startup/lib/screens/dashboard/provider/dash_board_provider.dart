...
class DashBoardProvider extends ChangeNotifier {
  ...
  addProduct() async {
    try {
      if (selectedMainImage == null && selectedCategory?.sId == null) {
      ...
      if (response.isOk) {
        ...
        if (apiResponse.success == true) {
          ...
          SnackBarHelper.showErrorSnackBar(
              'An error occurred ${apiResponse.message}');
        }
      } else {
        SnackBarHelper.showErrorSnackBar(
            'Internet connection issue ${response.statusText}');
      }
    } catch (e) {
  ...
  deleteProduct(Product product) async {
    try {
      ...
      if (response.isOk) {
        ApiResponse apiResponse = ApiResponse.fromJson(response.body, null);
        if (apiResponse.success == true) {
          SnackBarHelper.showSuccessSnackBar('Product deleted succesfully');
          _dataProvider.getALlProduct();
        } else {
          SnackBarHelper.showErrorSnackBar('Error deleting product ${apiResponse}');
        }
      } else {
        SnackBarHelper.showErrorSnackBar('Error connecting ${response.statusText}');
      }
    } catch (e) {
      SnackBarHelper.showErrorSnackBar('$e');
      rethrow;
  ...
  filterSubcatgory(Category category) {
