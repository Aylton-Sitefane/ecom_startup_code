      ...
      if (orderForUpdate != null) {
        ...
        if (response.isOk) {
          ...
          if (apiResponse.success == true) {
            ...
            SnackBarHelper.showErrorSnackBar('Falha ao actualizar pedido ${apiResponse.message}');
          }
        } else {
  ...
  deleteOrder(Order order) async {
    try {
      ...
      if (response.isOk) {
        ApiResponse apiResponse = ApiResponse.fromJson(response.body, null);
        if (apiResponse.success == true) {
          SnackBarHelper.showSuccessSnackBar('Pedido deletado com sucesso!');
          _dataProvider.getAllOrders();
        }
      } else {
        SnackBarHelper.showErrorSnackBar('Erro ${response.body?['message'] ?? response.statusText}');
      }
    } catch (e) {
      print(e);
      rethrow;
